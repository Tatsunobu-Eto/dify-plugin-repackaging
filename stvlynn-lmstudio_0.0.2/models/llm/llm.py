...
class LmstudioLargeLanguageModel(LargeLanguageModel):
    ...
    def _invoke_error_mapping(self, error: Exception) -> Exception:
        ...
        elif any(keyword in error_messages for keyword in ["Unauthorized", "API key", "authentication"]):
            return InvokeAuthorizationError("Authorization error: " + error_messages)
        elif any(keyword in error_messages for keyword in ["Bad request", "Invalid request", "validation"]):
            return InvokeBadRequestError("Bad request error: " + error_messages)
        elif any(keyword in error_messages for keyword in ["Too many requests", "Rate limit", "quota"]):
            return InvokeRateLimitError("Rate limit error: " + error_messages)
        elif any(keyword in error_messages for keyword in ["Server error", "Internal server error", "unavailable"]):
